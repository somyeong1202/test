...
public class PlayerMove : MonoBehaviour
{
    ...
    CapsuleCollider2D capsueCollider;  // 캐릭터 때문에 만듬 !!
    ...
    private void Awake()
    {
        ...
        capsueCollider = GetComponent<CapsuleCollider2D>(); // 추가
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            // Point
            bool isApple = collision.gameObject.name.Contains("Apple");    // 사과를 먹었을때 
            bool isStrawBerry = collision.gameObject.name.Contains("StrawBerry");
            bool isMushroom = collision.gameObject.name.Contains("Mushroom");
        ...
        else if(collision.gameObject.tag == "Finesh")
        {
            // Next Stage
    ...
    {
        // Health Down
        gameManager.HealthDown(); // 적에게 붙이치면 피가 1감소 !!
